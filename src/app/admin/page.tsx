...
export default function AdminLinkGenerator() {
  ...
  const generateLink = async () => {
    ...
    try {
      ...
      // D'ABORD créer le lien dans la table links (SANS signature)
      const newLink = {
        id,
        name: name.trim(),
        is_couple: isCouple,
      ...
      // ENSUITE, créer l'enregistrement dans wedding_invitations
      const participants = [
        { 
          name: name.trim(), 
          number: DEFAULT_PHONE_NUMBER, 
  ...
  const deleteLink = async (id: string) => {
    if (confirm('Are you sure you want to delete this link?')) {
      try {
        ...
        // Delete from wedding_invitations first
        const { error: invitationError } = await supabase
          .from('wedding_invitations')
          .delete()
          .eq('id', id)
  ...
  const shareLink = (url: string, name: string) => {
    ...
    if (navigator.share) {
      navigator.share({
        text: `Bonjour ${name}, Veuillez cliquer sur le lien ci après pour accéder à votre invitation pour le mariage du Couple Kitenge. \n\nQuestions ? Contactez-nous au : ${SUPPORT_PHONE} \n ${SUPPORT_PHONE2} \n\n Lien :`,
        url: url,
      }).catch(() => {
        copyLink(url);
      });
